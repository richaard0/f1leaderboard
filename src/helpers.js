...
function addInitialData() {
  ...
  if (
    ...
    !localStorage.getItem("allLapTimes")
  ) {
...
function calculateGapToFastestLap(fastestLap, lapTime) {
  const fastestLapTimeInMs =
    fastestLap.time.minutes * 60 * 1000 +
    fastestLap.time.seconds * 1000 +
    fastestLap.time.fractions;
...
function addRowToTable(lapData, fastestLap, tableRoot) {
  ...
  row.timeCell.innerText = `${lapData.time.minutes}:${lapData.time.seconds.toLocaleString("en-US", {
    ...
    useGrouping: false,
  })}:${lapData.time.fractions.toLocaleString("en-US", {
    minimumIntegerDigits: 3,
    useGrouping: false,
  })}`;
  row.gapCell.innerText =
    calculateGapToFastestLap(fastestLap, lapTime) === 0
      ? "---"
      : `+${((calculateGapToFastestLap(fastestLap, lapTime) / 1000) * -1).toFixed(3)}`;
...
function addRowToModalTable(lap, lapCount, driverFastestLap) {
  ...
  rowCells.gapCell.innerText =
    calculateGapToFastestLap(driverFastestLap, lap.time) === 0
      ? "---"
      : `+${((calculateGapToFastestLap(driverFastestLap, lap.time) / 1000) * -1).toFixed(3)}`;
...
function displayDataModalTable() {
  ...
  if (localStorage.getItem("lapOrder") === "asc") {
    ...
    localStorage.getItem("lapOrder") === "timeAsc" ||
    localStorage.getItem("lapOrder") === "timeDesc"
  ) {
...
function setModalTitle(driverLaps) {
  ...
  modalTitle.innerText = `${driverLaps[0].driverNumber} - ${getDriverName(
    driverLaps[0].driverNumber
  )} - ${getDriverTeam(driverLaps[0].driverNumber)}`;
}
...
function handleEditLapTimeModal(rowCells, lap, editIcon) {
  rowCells.lapTimeCell.isContentEditable
    ? (rowCells.lapTimeCell.contentEditable = "false")
    : (rowCells.lapTimeCell.contentEditable = "true");
  rowCells.lapTimeCell.classList.toggle("editable");
  rowCells.tyreCell.isContentEditable
    ? (rowCells.tyreCell.contentEditable = "false")
    : (rowCells.tyreCell.contentEditable = "true");
...
function createEditIcon(rowCells, lap) {
  ...
  editIcon.addEventListener("click", function () {
    // not using arrow function to access "this"
...
function displayEvents() {
  ...
  events.forEach((event) => {
    ...
    let top3LapTimes = fastestEventLaps
      .sort((a, b) => {
        return (
          a.time.minutes * 60 * 1000 +
          a.time.seconds * 1000 +
    ...
    for (let i = 0; i < top3LapTimes.length; i++) {
      ...
      lapTime.innerHTML = `${i + 1}. ${driverName}  ${leftPad(
        lap.time.minutes.toString(),
        "&nbsp;",
        22 - driverName.length
      )}:${lap.time.seconds.toLocaleString("en-US", {
        minimumIntegerDigits: 2,
        useGrouping: false,
      })}:${lap.time.fractions.toLocaleString("en-US", {
        minimumIntegerDigits: 3,
